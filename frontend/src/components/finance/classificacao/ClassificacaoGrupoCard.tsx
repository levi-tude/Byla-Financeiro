  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h3 className="byla-sensitive-content font-semibold text-slate-900 dark:text-slate-100">{titulo}</h3>
          <p className="byla-sensitive-content text-sm text-slate-600 dark:text-slate-400">{resumo}</p>
          {meta ? <p className="byla-sensitive-content text-xs text-slate-500">{meta}</p> : null}
        </div>
      ...
      {cartaoDetalhe && estado === 'pendente' && (
        <p className="byla-sensitive-content mt-2 text-xs text-amber-800 dark:text-amber-200">{cartaoDetalhe}</p>
      )}
      {sugestaoHint && estado === 'pendente' && (
        <p className="byla-sensitive-content mt-2 text-xs text-indigo-700 dark:text-indigo-300">{sugestaoHint}</p>
      )}
