...
const VINCULO_FILTRO_OPCOES: Array<{ id: FinancasAlunoVinculoFiltro; label: string }> = [
  { id: 'todos', label: 'Todos' },
  { id: 'vinculado', label: 'Com reconhecimento' },
  { id: 'sem_vinculo', label: 'Sem reconhecimento' },
];
...
export function FinancasAlunosPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4 p-6">
      <Topbar
        title="Alunos"
        subtitle="Pagamentos previstos no Fluxo da competência — reconhecimento no banco e situação de cobrança"
      />
      ...
      <div className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-900/40 dark:bg-sky-950/30 dark:text-sky-100">
        <p>
          ...
          <span className="font-medium">pendente</span> se o crédito caiu fora do mês da competência.
        </p>
      </div>
      ...
      {!query.isLoading && grupos.length > 0 ? (
        <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-200">
          ...
          <span className="font-semibold">{resumo.comReconhecimento}</span> com reconhecimento ·{' '}
        ...
        <EmptyState
          message={
            grupos.length === 0
                ...
                ? 'Nenhum pagamento sem reconhecimento no banco neste mês.'
                : vinculoFiltro === 'vinculado'
                  ? 'Nenhum pagamento com reconhecimento no banco neste mês.'
                      ...
                      <table className="min-w-full text-left text-sm">
                        <thead className="border-y border-slate-100 bg-slate-50/80 text-xs uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-400">
                          <tr>
                            ...
                            <th className="px-4 py-2 font-medium">Reconhecimento no banco</th>
                            <th className="px-4 py-2 font-medium">Situação de pagamento</th>
